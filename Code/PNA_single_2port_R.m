...
fopen(s1);
fopen(obj1);
NOP = 201;%32001; % number of points
...
initializePNA(obj1,meas_name,cal_name,NOP);
...
for iter = 1:N
   ...
   % fprintf(s1,['I',num2str(DIRECTION*64*12000/N),',1700,0,0,5000,5000,1000,0,1000,1000,50,64']); pause(1+1/1700*60*(12000/N)); % move the steppermotor counterclockwise by 1.8 degrees (1.8*4 was replaced by 360/N to make the code adjust ensemble as the user sets it.
    fprintf(s1,['I',num2str(DIRECTION*64*12000/50),',1700,0,0,5000,5000,1000,0,1000,1000,50,64']); pause(1+1/1700*60*(12000/50)); % move the steppermotor counterclockwise by 1.8 degrees (1.8*4 was replaced by 360/N to make the code adjust ensemble as the user sets it.
   % fprintf(obj1, 'CALC:TRAN:TIME:STATE OFF'); % turn off transfrom (to time domain)
    %fprintf(obj1, 'INIT:IMM'); % send trigger to initiate one sweep
    %fprintf(obj1, '*WAI'); % wait until sweep is complete
    ...
    [Freq,S11,S12,S21,S22] = getSparametersFrequencyDomain(obj1,NOP,false);
    SCf(:,1,iter) = S11;%S11R + 1i*S11I; %One port line
    SCf(:,2,iter) = S21;%S21R + 1i*S21I;
    SCf(:,3,iter) = S12;%S12R + 1i*S12I;
    SCf(:,4,iter) =S22;% S22R + 1i*S22I;
    Time = toc;
    ...
    for i = 1:10
       % fprintf(obj1, 'CALC:FILT:TIME:STATE ON'); % tunn on gating is on
        ...
        [Freq,S11,S12,S21,S22] = getSparametersFrequencyDomain(obj1,NOP,true,i,l);
        
        Srad(:,2,iter) = S11;
        Srad(:,2,iter) = S21;%S21R + 1i*S21I;
        Srad(:,3,iter) = S12;%S12R + 1i*S12I;
        Srad(:,4,iter) = S22;%S22R + 1i*S22I;
        Srad(:,1,iter,i) = S11;%S11R + 1i*S11I; %2 Port Data Collected
        Srad(:,2,iter,i) = S12;%S12R + 1i*S12I;
        Srad(:,3,iter,i) = S21;%S21R + 1i*S21I;
        Srad(:,4,iter,i) = S22;%S22R + 1i*S22I;
        Time = toc;
    end
    ...
    [t,S11,S12,S21,S22] = getSparametersTimeDomain(obj1,meas_name);
    SCt(:,1,iter) = S11;
    SCt(:,2,iter) = S12;
    SCt(:,3,iter) = S21;
    SCt(:,4,iter) = S22;
